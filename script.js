(function(){
  ...
  const root=document.documentElement,forced=new URLSearchParams(location.search).get("theme"),saved=forced||localStorage.getItem("flowbalance-theme");
  if(saved==="dark"||(!saved&&matchMedia("(prefers-color-scheme: dark)").matches))root.classList.add("dark");
  if(saved==="light")root.classList.remove("dark");
  $("#theme-toggle")?.addEventListener("click",()=>{root.classList.toggle("dark");localStorage.setItem("flowbalance-theme",root.classList.contains("dark")?"dark":"light")});
